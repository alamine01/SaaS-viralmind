...
async function checkEndpoint(pathName, method) {
  ...
  try {
    ...
    if (status === 200 && !text.includes("does not exist")) {
      ...
      console.log(`Endpoint ${method} /${pathName} -> Status: ${status} | Response snippet: ${text.substring(0, 200)}`);
    }
  } catch (e) {
...
async function start() {
  ...
  console.log("Starting probe of get_ig_ endpoints...");
